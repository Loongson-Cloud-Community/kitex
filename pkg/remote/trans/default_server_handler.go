...
func (t *svrTransHandler) Read(ctx context.Context, conn net.Conn, recvMsg remote.Message) (err error) {
	...
	if codec, ok := t.codec.(remote.MetaDecoder); ok {
		if err = codec.DecodeMeta(ctx, recvMsg, bufReader); err == nil {
			t.startMetaProfiler(ctx, recvMsg)
			err = codec.DecodePayload(ctx, recvMsg, bufReader)
			t.startPayloadProfiler(ctx, recvMsg)
		}
	} else {
...
func (t *svrTransHandler) startMetaProfiler(ctx context.Context, msg remote.Message) {
	if t.opt.ProfilerCtl == nil {
		return
	}
	t.opt.ProfilerCtl.TagMeta(ctx, msg)
}

func (t *svrTransHandler) startPayloadProfiler(ctx context.Context, msg remote.Message) {
	if t.opt.ProfilerCtl == nil {
		return
	}
	t.opt.ProfilerCtl.TagPayload(ctx, msg)
}
